...
extDir="../extensions/rslte/conf/"
backupsufix="_withoutID"
idSufix="_ids"
reportFileName="rslte_report.xml"
levelFileName="rslte_level.xml"
neVersion="rslte_RL70"

CONTENT3=$VARROOT/global/shared/content3

NEW_EXT=`grep -H rsVersion \`find  $CONTENT3/rslte_*  | grep extensions | grep engine_ext.properties \` /dev/null | sort -n -t '=' -k 2 | cut -d ':' -f 1 | tail -n 1  | sed  's/.*\(rslte_.*\)/\1/'| cut -d '/' -f 1`
...
if [ "$neVersion" != "$NEW_EXT"  ]; then
	...
	echo
	echo $neVersion is not the newer LTE version installed 
